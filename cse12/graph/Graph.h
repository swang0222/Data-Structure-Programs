...
class Graph {
    private:
        // Store all the edges in tuples (first_node, second_node, weight)
        vector<tuple<string, string, double>> edges;
        // A set that contains all nodes (to make sure no duplicates)
        unordered_set<string> nodes_set;
        // Used to construct DisjointSet
        unordered_map<string, int> node_indices;
...
class DisjointSet {
    public:
        ...
        //return the sentinel of the node in the corresponding index
        int find(int x) {
            if (parent[x] != x) {
